...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (storedProfile) {
      try {
        ...
        setStage(parsedProfile.stage ?? "");
        setDistrict(parsedProfile.district ?? "");
        setLanguage(parsedProfile.language ?? "English");
        setFirstChild(parsedProfile.firstChild ?? "");
      } catch {
        // keep defaults
      }
    }
  ...
  function handleReset() {
    localStorage.removeItem("parentingViennaProfile");
    router.push("/");
  }
    ...
    <main className="mx-auto min-h-screen max-w-md bg-[#FFF6FA] px-4 py-6 pb-24">
      ...
      <div className="rounded-3xl bg-white p-5 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-pink-500">Your profile</p>
            <h1 className="mt-2 text-3xl font-bold text-gray-900">
              Family settings
            </h1>
            <p className="mt-2 text-gray-600">
              Update your details to personalize guidance in the app.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-5">
          ...
          <button
            type="button"
            onClick={handleReset}
            className="w-full rounded-2xl border border-pink-100 bg-white px-4 py-3 font-semibold text-gray-700"
          >
